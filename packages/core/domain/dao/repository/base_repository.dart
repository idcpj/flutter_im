...
abstract class BaseRepository {
  ...
  Future<int> update<T extends Table>(T item) async {
    ...
    return await db.update(item.tableName, item.toMap(),
        where: '${item.primaryKey.name} = ?',
        whereArgs: [item.toMap()[item.primaryKey.name]]);
  }
  ...
  Future<int> delete<T extends Table>(T item) async {
    ...
    return await db.delete(item.tableName,
        where: '${item.primaryKey.name} = ?',
        whereArgs: [item.toMap()[item.primaryKey.name]]);
  }
  ...
  Future<T?> findById<T extends Table>(T item) async {
    if (item.primaryKey.name.isEmpty) {
      throw ArgsException(
          '[Repository] findById primaryKey is empty ${item.tableName}');
    }
    final data = await db.query(item.tableName,
        where: '${item.primaryKey.name} = ?',
        whereArgs: [item.toMap()[item.primaryKey.name]]);
    if (data.isEmpty) {
      throw ArgsException(
          '[Repository] findById data is empty ${item.tableName}');
    }
